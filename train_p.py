...
def training_loop(model,
    ...
    for epoch in range(n_epoch, config.epochs):
        ...
        for batch in train_loader:
            ...
            if n_iter % config.n_save_states_iter == 0:
                save_states(f'states.pth', model.module, optimizer,
                            n_iter, epoch, config)  # Use model.module when saving checkpoints with DDP

            if n_iter % config.n_save_backup_iter == 0 and n_iter > 0:
                save_states(f'states_{n_iter}.pth', model.module,
                            optimizer, n_iter, epoch, config)  # Use model.module when saving checkpoints with DDP

            n_iter += 1

        # VALIDATE
        val_loss = validate(model.module, test_loader, writer, device, n_iter)  # Use model.module for validation
        print(f"Validation loss: {val_loss}")
